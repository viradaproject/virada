...
export default function ViradaPrototype() {
  ...
  const registerUser = (person) => {
    ...
    const targetClub = clubs.find(c => c.code === code);
    if (!targetClub) { setLoginError(clubs.length === 0 ? "Todavía no se ha registrado ningún club." : "Código de club incorrecto."); return; }
    if (isUsernameTaken(person.username)) { setLoginError("Ese nombre de usuario ya existe. Elige otro."); return; }
    const id = `u${Date.now()}`;
    const entry = { id, clubId: targetClub.id, username: person.username, apodo: person.apodo, side: person.side };
    setPendingUsers(prev => [...prev, entry]);
    setPasswords(prev => ({ ...prev, [id]: "1234" }));
    setLastRegistered(entry);
    setCurrentClubId(targetClub.id);
    setScreen("pendingRole");
  };
  const assignPendingUser = (id, role, teamId) => {
  ...
  const loginUser = (username, password) => {
    setLoginError(null);
    if (isAdminLogin(username, password)) { setRole("admin"); setCurrentClubId(null); setScreen("home"); return; }
    const found = findLoginId(username);
    if (!found) { setLoginError("Usuario o contraseña incorrectos."); return; }
    if (found.pending) { setLastRegistered(found.pending); setCurrentClubId(found.pending.clubId); setScreen("pendingRole"); return; }
    const expected = passwords[found.id] ?? "1234";
    if (password !== expected) { setLoginError("Usuario o contraseña incorrectos."); return; }
    setCurrentUserId(found.id);
    setCurrentClubId(found.clubId ?? null);
    setRole(roleOf(found.id));
    setScreen("home");
  };

  const registerClub = (name, username, password) => {
    setLoginError(null);
...
function LoginScreen({ onRegisterClub, onLoginClub, onLoginUser, onRegisterUser, onRecoverPassword, onClearError, loginError, Logo }) {
  ...
  const submitRegisterUser = () => {
    onRegisterUser({ username: usernameInput, apodo: apodoInput, side: regSide, clubCode: clubCodeInput });
  };
